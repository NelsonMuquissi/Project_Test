...
async function seedData() {
    try {
        ...
        await ReconciliationJob.destroy({ where: { companyId: company.id } });
        console.log(`Gerando 7 dados realistas para a empresa ${company.name}...`);

        for (let i = 0; i < 7; i++) {
            const date = new Date();
            date.setDate(date.getDate() - (i * 2));
            const matchCount = Math.floor(Math.random() * 50) + 15;
            ...
            const txCount = matchCount + Math.floor(Math.random() * 10);

            for (let j = 0; j < txCount; j++) {
                const isMatched = j < matchCount;
            ...
            for (let m = 0; m < matchCount; m++) {
                await ReconciliationMatch.create({
                    ...
                    status: 'confirmed',
                    createdAt: date
                }).catch(e => { }); // Ignoring minor constraint issues
            }
        }
        console.log('Seed de dados 100% finalizado e associado para', companyName);
    } catch (err) {
